...
def loss_function(tr_pred, rot_pred, tor_pred, data, t_to_sigma, device, tr_weight=1, rot_weight=1,
    ...
    #tr_sigma, rot_sigma, tor_sigma = t_to_sigma(*[torch.cat([d.complex_t[noise_type] for d in data]) if device.type == 'cuda' else data.complex_t[noise_type]
    tr_sigma, rot_sigma, tor_sigma = t_to_sigma(*[ data.complex_t[noise_type] for noise_type in ['tr', 'rot', 'tor']])
    tr_sigma = tr_sigma.cpu()
    rot_sigma = rot_sigma.cpu()
    tor_sigma = tor_sigma.cpu()

    mean_dims = (0, 1) if apply_mean else 1

    # translation component
    tr_score = torch.cat([d.tr_score for d in data], dim=0) if device.type == 'cuda' else data.tr_score.to(device1)
    tr_sigma = tr_sigma.unsqueeze(-1)
    tr_loss = ((tr_pred.to(device1) - tr_score) ** 2 * tr_sigma ** 2).mean(dim=mean_dims)
    tr_base_loss = (tr_score ** 2 * tr_sigma ** 2).mean(dim=mean_dims).detach()

    # rotation component
    rot_score = torch.cat([d.rot_score for d in data], dim=0) if device.type == 'cuda' else data.rot_score.to(device1)
    rot_score_norm = so3.score_norm(rot_sigma.to(device1) ).unsqueeze(-1)

    rot_loss = (((rot_pred.to(device1) - rot_score) / rot_score_norm) ** 2).mean(dim=mean_dims)
    ...
    if not no_torsion:
        ...
        tor_score = torch.cat([d.tor_score for d in data], dim=0) if device.type == 'cuda' else data.tor_score.to(device1)
        ...
        if apply_mean:
            ...
            index = torch.cat([torch.ones(d['ligand'].edge_mask.sum()) * i for i, d in
                ...
                data['ligand', 'ligand'].edge_index[0][data['ligand'].edge_mask]].to(device1)
    ...
    for data in tqdm(loader, total=len(loader)):
        try:
            ...
            if test_sigma_intervals > 0:
                #liuxg
                complex_t_tr, complex_t_rot, complex_t_tor = [data.complex_t[noise_type] for noise_type in ['tr', 'rot', 'tor']]
                #[torch.cat([d.complex_t[noise_type] for d in data]) for noise_type in ['tr', 'rot', 'tor']]
