...
class TensorProductScoreModel(torch.nn.Module):
    ...
    def forward(self, data):
        if not self.confidence_mode:
            ...
            #    typev = [d.complex_t[noise_type] for d in data ]
            #    typev  = torch.hstack(typev)
            #    input_t.append(typev)


            #tr_sigma, rot_sigma, tor_sigma = self.t_to_sigma(*input_t)
            #print('xx--------------')
...
class GaussianSmearing(torch.nn.Module):
    ...
    def forward(self, dist):
        dist = dist.view(-1, 1) - self.offset.to(dist.device).view(1, -1)
